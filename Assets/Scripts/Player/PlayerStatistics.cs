...
public class PlayerStatistics : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        CalculateSafety(); // safety calculation
    }
    ...
    public void CalculateSafety()
    {
        ...
        isSafe = false; // reset safety status
        foreach (GameObject safeZone in safeZones)
        {
            //// check if player is within the safe zone's collider
            //if (col == null) continue;
            //if (col.OverlapPoint(playerPos2D))
            //{
            //    isSafe = true;
            //    break;
            //}
            Debug.Log("Checking safe zone: " + safeZone.name);
            if (safeZone.GetComponent<BoxCollider2D>().OverlapPoint(gameObject.transform.position))
            {
                isSafe = true;
                break; // no need to check other safe zones
            }
        }
    }
}
